	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < graph.length; i++) {
			if (!graph[i].visited) {
				...
				List<Vertex> verticesInComponents = new ArrayList<>();
				dfs(graph[i], verticesInComponents);
				
				sb.append(graph[i].id).append(" ").append(verticesInComponents.size()).append("\n");				
			}
		}
		System.out.println(sb);
	}
	
	static void dfs(Vertex v, List<Vertex> verticesInComponents) {
		v.visited = true;
		verticesInComponents.add(v);
		for (Vertex w : v.adjecentVertices) {
			if (w.visited == false) {
				dfs(w,verticesInComponents);
			}
		}
	}
