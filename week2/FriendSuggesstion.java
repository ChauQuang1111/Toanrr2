...
public class FriendSuggesstion {
	...
	public static void main(String[] args) {
		...
		int nStudents = ni();
		int nFriendships = ni();
		int minimumFriends = ni();
		
		Vertex[] graph = readGraph(nStudents, nFriendships);
		
		for (int i = 0; i < graph.length; i++) {
			sb.append(graph[i].id).append(" ");
			
			for (var j : graph[i].adjecentVertices) {
				if (j.getDegree() < minimumFriends) {
	...
	static Vertex[] readGraph(int nStudents, int nFriendShips) {
		Vertex[] vertices = new Vertex[nStudents];
		...
		for (Vertex v : vertices) {
			v.adjecentVertices.sort((v1, v2) -> Integer.compare(v1.id, v2.id));
		}
		return vertices;
	}
